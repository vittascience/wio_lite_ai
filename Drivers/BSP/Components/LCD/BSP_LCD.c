...
EXTMEM uint8_t  LCD_FRAME_BUFFER[320 * 240 * 2];
...
void LCD_Frame_Draw_Rectangle(uint16_t start_x, uint16_t start_y, uint16_t width, uint16_t height, uint8_t rotation)
{
	uint16_t color = 5123;
	for ( uint16_t x = start_x; x < start_x + width; x++)
	{
		LCD_Frame_Draw_Point(x, start_y, color, rotation);
		LCD_Frame_Draw_Point(x, start_y + height, color, rotation);
	}

	for ( uint16_t y = start_y; y < start_y + height; y++)
	{
		LCD_Frame_Draw_Point(start_x, y, color, rotation);
		LCD_Frame_Draw_Point(start_x + width, y, color, rotation);
	}
}
...
void LCD_Frame_Draw_String(uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint8_t size,char *p, uint8_t rotation)
{
    ...
    while((*p<='~')&&(*p>=' '))
    {
        if(x>=width){x=x0;y+=size;}
        if(y>=height)break;
        LCD_Frame_Draw_Char(x,y,*p,size,0, rotation);
        x+=size/2;
        p++;
    }
}
